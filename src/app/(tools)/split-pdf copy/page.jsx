...
const PDFPreview = memo(({ file, pageNumber = 1, width = 250, className = "" }) => {
...
export default function SplitPDFPage() {
  ...
  const [sizeInputValue, setSizeInputValue] = useState("25") // For controlled input
  ...
  // Handle size input blur (when user finishes typing)
  ...
  const createStableFileData = useCallback(async (file) => {
    ...
    try {
      ...
      const stableData = { blob, dataUrl, uint8Array: uint8Array.slice() }
  ...
  const handleFiles = async (newFiles) => {
    ...
    setFile(fileObject)
    setLoadingPdf(true)
    setPdfPages(0) // Reset page count while loading
    setPdfHealthCheck(true)
  }

  const removeFile = () => {
    setLoadingPdf(false)
    if (fileDataCache.current) fileDataCache.current = null
  ...
  const onDocumentLoadSuccess = (pdf) => {
    ...
    // Update initial range with total pages
    if (rangeMode === "custom") {
      setRanges([{ id: 1, from: 1, to: pdf.numPages }])
    }

    // Initialize all pages selection
  ...
  const togglePageSelection = useCallback(
    (pageNum) => {
      setSelectedPages((prev) => {
        ...
        // Auto-switch to select mode if not all pages are selected
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      if (response.data) {
        const downloadUrl = `/downloads/document=${response.data.data.splitFiles}?dbTaskId=${response.data.data.dbTaskId}`
  ...
  return (
    <div className="md:h-[calc(100vh-82px)]">
      {/* Add back the hidden Document component for page count */}
      {file && (
        <Document
          file={file.stableData.dataUrl}
...
// import { useState, useRef, useCallback, useEffect, useMemo, memo } from "react"
// import { useRouter } from "next/navigation"
// import { FileText, X, ArrowRight, Plus, Move } from "lucide-react"
// import { Document, Page, pdfjs } from "react-pdf"
// import ProgressScreen from "@/components/tools/ProgressScreen"
// import FileUploader from "@/components/tools/FileUploader"
// import Api from "@/utils/Api"
...
//   // Handle size input validation
//   const handleSizeChange = useCallback(
//     (value) => {
//       const numValue = Number.parseInt(value) || 1

//       if (sizeUnit === "MB") {
//         if (numValue > fileSizeInMB) {
//           setMaxSize(Math.floor(fileSizeInMB))
//         } else {
//           setMaxSize(numValue)
//         }
//       } else {
...
//       if (newUnit === "KB" && sizeUnit === "MB") {
//         setMaxSize(maxSize * 1024)
//       } else if (newUnit === "MB" && sizeUnit === "KB") {
//         setMaxSize(Math.max(1, Math.floor(maxSize / 1024)))
//       }
//       setSizeUnit(newUnit)
//     },
//     [maxSize, sizeUnit],
...
//             onClick={() => togglePageSelection(pageNum)}
//           >
//             <div className="relative h-40 flex justify-center items-center bg-gray-100 rounded-lg mb-3">
//               {selectedPages.has(pageNum) && (
//                 <div className="absolute top-2 left-2 w-6 h-6 bg-green-500 rounded-full flex items-center justify-center z-10">
//                   <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
//                     <path
...
//           <div className="flex items-center justify-between mb-6">
//             <h2 className="text-2xl font-bold text-gray-900">
//               {activeTab === "range" && rangeMode === "custom" && "Range 1"}
//               {activeTab === "range" && rangeMode === "fixed" && `${fixedRanges.length} Ranges`}
//               {activeTab === "pages" && `${selectedPages.size} Pages Selected`}
//               {activeTab === "size" && "Split by Size"}
//             </h2>
...
//         {/* Sidebar */}
//         <div className="col-span-3 overflow-y-auto border-l flex flex-col">
//           <div className="p-6 border-b">
//             <h3 className="text-2xl font-bold text-gray-900 mb-4 text-center">Split</h3>
...
//               <button
//                 onClick={() => setActiveTab("range")}
//                 className={`flex-1 p-3 rounded-lg text-sm font-medium transition-colors duration-200 flex flex-col items-center gap-2 ${activeTab === "range"
//                     ? "bg-green-100 text-green-800 border-2 border-green-300"
//                     : "bg-gray-100 text-gray-600 hover:bg-gray-200"
//                   }`}
//               >
...
//               <button
//                 onClick={() => setActiveTab("pages")}
//                 className={`flex-1 p-3 rounded-lg text-sm font-medium transition-colors duration-200 flex flex-col items-center gap-2 ${activeTab === "pages"
//                     ? "bg-green-100 text-green-800 border-2 border-green-300"
//                     : "bg-gray-100 text-gray-600 hover:bg-gray-200"
//                   }`}
//               >
...
//               <button
//                 onClick={() => setActiveTab("size")}
//                 className={`flex-1 p-3 rounded-lg text-sm font-medium transition-colors duration-200 flex flex-col items-center gap-2 ${activeTab === "size"
//                     ? "bg-green-100 text-green-800 border-2 border-green-300"
//                     : "bg-gray-100 text-gray-600 hover:bg-gray-200"
//                   }`}
//               >
...
//                       type="number"
//                       min="1"
//                       max={sizeUnit === "MB" ? Math.floor(fileSizeInMB) : Math.floor(fileSizeInMB * 1024)}
//                       value={maxSize}
