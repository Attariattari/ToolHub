...
const PDFDocumentPreview = memo(({ file, isLoading, onLoadSuccess, onLoadError, pageCount }) => (
...
const PDFPreview = memo(({ file, isLoading, onLoadSuccess, onLoadError, onRemove, isHealthy, pageCount }) => {
  ...
  const renderPreview = () => {
    ...
    if (file.type === "application/pdf" && file.stableData) {
      return <PDFDocumentPreview file={file} isLoading={isLoading} onLoadSuccess={handleLoadSuccess} onLoadError={handleLoadError} pageCount={pageCount} />
    }
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 flex-1">
      ...
      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Convert your PDF files to PowerPoint presentations. Each page will be converted to a slide with high quality preservation.
        </p>
      </div>
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      {!limitsExceeded.hasAnyExceeded ? (
        ...
        >
          Convert to PowerPoint <ArrowRight className="w-5 h-5" />
        </button>
      ) : (
...
export default function PDFToPowerPointPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=pdf-to-powerpoint`
  ...
  if (files.length === 0) {
    return (
      <SafeFileUploader
        ...
        pageSubTitle="Convert your PDFs to PowerPoint presentations."
  ...
  const sidebarProps = {
    files, totalSize, totalPages, hasUnhealthyFiles, limitsExceeded, onConvert: handleConvert
  }
          ...
          <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => setShowMobileSidebar(false)}>
            <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-xl overflow-y-auto custom-scrollbar pb-16" onClick={(e) => e.stopPropagation()}>
              <div className="p-4 border-b flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-900">PDF to PowerPoint</h3>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                {!limitsExceeded.hasAnyExceeded ? (
                  ...
                  >
                    Convert to PowerPoint <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
