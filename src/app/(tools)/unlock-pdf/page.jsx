...
import { FileText, X, ArrowRight, Unlock, Settings } from "lucide-react"
        ...
        <div className="w-full h-full bg-gray-50 flex flex-col items-center justify-center rounded-lg">
          <div className="relative">
            <div className="w-16 h-20 bg-blue-600 rounded-lg flex items-center justify-center relative">
              <IoMdLock className="w-10 h-10 text-white" />
  ...
  return (
    <div ref={elementRef} className={`bg-white rounded-xl border-2 transition-all duration-200 overflow-hidden relative ${borderClasses}`}>
      <div className="relative h-56 p-3 pt-10">
        ...
        {isPasswordProtected && (
          <div className="absolute top-2 left-2 bg-blue-500 text-white text-xs px-2 py-1 rounded-full font-medium flex items-center gap-1">
...
const FileInfoSection = ({ files, totalSize, passwordProtectedCount }) => (
  <div className="mb-6">
    ...
    <div className="space-y-2 text-sm">
      {[
        ...
        ["Password protected:", passwordProtectedCount],
        ["Output format:", "Unlocked PDF"]
      ].map(([label, value]) => (
...
const Sidebar = ({
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 flex-1">
      <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Unlock PDF</h3>

      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Remove password protection from your PDF files. Protected files will be automatically detected and unlocked during processing.
        </p>
      </div>
      ...
      {hasUnhealthyFiles && (
        <div className="bg-yellow-50 rounded-xl p-4 mb-6">
          <p className="text-sm text-yellow-800">
            Some files have preview issues but can still be unlocked. Check the yellow-highlighted files.
          </p>
        </div>
      )}
      ...
      {passwordProtectedCount > 0 && (
        <div className="bg-green-50 rounded-xl p-4 mb-6">
          <p className="text-sm text-green-800">
            {passwordProtectedCount} password-protected file{passwordProtectedCount > 1 ? "s" : ""} detected. These will be unlocked during processing.
          </p>
        </div>
      )}

      {files.length > 0 && <FileInfoSection files={files} totalSize={totalSize} passwordProtectedCount={passwordProtectedCount} />}
    </div>
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      ...
      {files.length === 0 && <p className="text-xs text-gray-500 text-center mt-2">Select PDF files to unlock</p>}
    </div>
  </div>
)
...
export default function UnlockPDFPage() {
  ...
  const handlePasswordSubmit = useCallback(async (passwords) => {
    ...
    try {
      ...
      const response = await Api.post("/tools/unlock", formData, {
      ...
      if (response.data) {
        ...
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=unlock-pdf`
        router.push(downloadUrl)
      } else {
        toast.error("No unlocked files received from server")
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || "Error unlocking files")
    } finally {
  ...
  const passwordProtectedCount = useMemo(() => passwordProtectedFiles.size, [passwordProtectedFiles])
  ...
  const sidebarProps = {
    files, totalSize, hasUnhealthyFiles, passwordProtectedCount, limitsExceeded, onUnlock: handleUnlock
  }
          ...
          <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => setShowMobileSidebar(false)}>
            <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-xl overflow-y-auto custom-scrollbar pb-16" onClick={(e) => e.stopPropagation()}>
              <div className="p-4 border-b flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-900">Unlock PDF</h3>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                {!limitsExceeded.hasAnyExceeded ? (
                  ...
                  >
                    Unlock PDF <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
...
// import { useState, useRef, useCallback, useEffect, useMemo, memo } from "react"
// import { useRouter } from "next/navigation"
// import { FileText, X, ArrowRight, Unlock } from "lucide-react"
// import { IoMdLock } from "react-icons/io"
// import { Document, Page, pdfjs } from "react-pdf"
// import ProgressScreen from "@/components/tools/ProgressScreen"
// import FileUploader from "@/components/tools/FileUploader"
...
//         {/* Unlock indicator for password-protected files */}
//         {isPasswordProtected && (
...
//   const [passwordProtectedFiles, setPasswordProtectedFiles] = useState(new Set())
//   const [showPasswordModal, setShowPasswordModal] = useState(false)
//   // Remove failedUnlockFiles state
//   // const [failedUnlockFiles, setFailedUnlockFiles] = useState([])
//   const [showMobileSidebar, setShowMobileSidebar] = useState(false)
...
//   // Handle password submission for password-protected files
...
//       if (response.data) {
//         const downloadUrl = `/downloads/document=${response.data.data.unlockedFiles}?dbTaskId=${response.data.data.dbTaskId}&tool-type=unlock-pdf`
//         router.push(downloadUrl)
//       } else {
//         toast.error("No unlocked files received from server")
//       }
...
//             <div className="bg-blue-50 rounded-xl p-4 mb-6">
//               <p className="text-sm text-blue-800">
//                 Remove password protection from your PDF files. Protected files will be automatically detected.
//               </p>
//             </div>
...
//                   detected. These will be unlocked during processing.
//                 </p>
//               </div>
//             )}

//             {passwordProtectedFiles.size === 0 && files.length > 0 && (
//               <div className="bg-gray-50 rounded-xl p-4 mb-6">
//                 <p className="text-sm text-gray-600">
//                   No password-protected files detected. All files appear to be unlocked already.
//                 </p>
//               </div>
//             )}
//           </div>
...
//                   <span className="font-semibold text-green-600">{passwordProtectedFiles.size}</span>
//                 </div>
//               )}
//             </div>
...
//                 {files.length === 0 && (
//                   <p className="text-xs text-gray-500 text-center mt-2">Select PDF files to unlock</p>
//                 )}
//               </div>
//             </div>
//           </div>
...
//       {/* Password Modal for password-protected files */}
//       <PasswordModal
//         isOpen={showPasswordModal}
//         onClose={() => setShowPasswordModal(false)}
//         passwordProtectedFiles={protectedFilesForModal}
