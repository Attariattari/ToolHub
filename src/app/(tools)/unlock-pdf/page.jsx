...
export default function UnlockPDFPage() {
  ...
  // Handle password submission for failed unlock files
  const handlePasswordSubmit = useCallback(
    async (passwords) => {
      ...
      try {
        ...
        // Send passwords for failed unlock files
        const filePasswords = {}
        failedUnlockFiles.forEach((file) => {
        ...
        if (response.data) {
          const downloadUrl = `/downloads/document=${response.data.data.unlockedFiles}?dbTaskId=${response.data.data.dbTaskId}`
        ...
        alert("Failed to unlock PDFs. Please try again.")
      } finally {
        setIsUploading(false)
      }
    },
    [files, failedUnlockFiles, router],
  )
  ...
  const handleUnlock = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) {
        // Check if some files failed to unlock
        if (response.data.data.failedFiles && response.data.data.failedFiles.length > 0) {
          // Map failed files back to our file objects
          const failedFileNames = response.data.data.failedFiles
          const failedFiles = files.filter((file) => failedFileNames.includes(file.name))
          setFailedUnlockFiles(failedFiles)
          setShowPasswordModal(true)
          setIsUploading(false)
          return
        }

        // All files unlocked successfully
        const downloadUrl = `/downloads/document=${response.data.data.unlockedFiles}?dbTaskId=${response.data.data.dbTaskId}`
        router.push(downloadUrl)
      } else {
        toast.error("No unlocked files received from server")
  ...
  return (
    <div className="md:h-[calc(100vh-82px)]">
      ...
      {/* Password Modal for failed unlock files */}
      <PasswordModal
        isOpen={showPasswordModal}
        onClose={() => setShowPasswordModal(false)}
        passwordProtectedFiles={failedUnlockFiles}
