...
const FileInfoSection = ({ currentFile, totalSize, totalPages }) => (
  <div className="mb-6">
    <h4 className="font-semibold text-red-900 mb-3">File Information</h4>
    <div className="space-y-2 text-sm">
      {[
        ["File name:", currentFile?.name || "No file selected"],
        ["File size:", `${totalSize} MB`],
        ["Total pages:", `${totalPages} pages`],
        ["File type:", "PDF Document"]
      ].map(([label, value]) => (
        <div key={label} className="flex items-center justify-between">
          <span className="text-red-700">{label}</span>
          <span className="font-semibold text-red-900 truncate ml-2">{value}</span>
        </div>
      ))}
    </div>
  </div>
...
const LimitsExceeded = ({ limitsExceeded, totalSize, totalPages }) => (
  <div className="bg-white rounded-xl border-2 border-red-200 p-4">
    ...
    <p className="text-xs text-red-600 text-center mt-3">Please select a smaller file or fewer pages to continue.</p>
  </div>
);
...
export default function PDFViewer() {
  ...
  const [deleteSpecificImageElement, setDeleteSpecificImageElement] = useState(null);
  const [allDrawElements, setAllDrawElements] = useState([]);
  const [clearAllDrawElements, setClearAllDrawElements] = useState(false);
  const [deleteSpecificDrawElement, setDeleteSpecificDrawElement] = useState(null);
  ...
  // Create stable file data with size and page limits check
    ...
    async (newFiles) => {
      ...
      const fileObjects = await Promise.all(
        fileArray.map(async (file, index) => {
          ...
          if (!stableData) {
            return null; // File rejected due to limits
          }
      ...
      if (validFiles.length > 0) {
        ...
        if (fileArray.length > 1) {
          toast.info("Only one PDF can be viewed at a time. Using the first valid file.");
        }
      }
    },
  ...
  const handleThumbnailClick = useCallback((pageNumber) => {
    ...
    setTimeout(() => {
      ...
      if (pageElement && mainViewerRef.current) {
        let scrollContainer = mainViewerRef.current.querySelector(".overflow-auto");
        if (!scrollContainer) {
        ...
        scrollContainer.scrollTo({
          top: scrollContainer.scrollTop + (elementRect.top - containerRect.top) - 50,
          behavior: "smooth",
        });
      }
    }, 50);
  ...
  // Computed values for limits checking
  const totalSize = useMemo(() => {
    if (!currentFile) return "0.00";
    return parseFloat(currentFile.size).toFixed(2);
  }, [currentFile]);

  const totalPages = useMemo(() => {
    return currentFilePages;
  }, [currentFilePages]);
  ...
  const limitsExceeded = useMemo(() => {
    ...
    return {
      exceedsSize,
      exceedsPages,
      hasAnyExceeded: exceedsSize || exceedsPages
    };
  ...
  const [currentLayout, setCurrentLayout] = useState("continuous");
  ...
  const fontOptions = ["Arial", "Times New Roman", "Courier New", "Helvetica", "Georgia", "Verdana", "Comic Sans MS", "Impact"];
  ...
  const current = options.find((opt) => opt.value === textEditingState.selectedAlignment);
  ...
  // Build and send FormData to backend for edit-pdf
  const getReferenceDimensions = () => {
    try {
      if (!mainViewerRef.current) return { width: 800, height: 1120 };
      const pageEl = mainViewerRef.current.querySelector('.react-pdf__Page');
      if (!pageEl) return { width: 800, height: 1120 };
      const rect = pageEl.getBoundingClientRect();
      return { width: (rect.width * 100) / zoom, height: (rect.height * 100) / zoom };
    } catch {
      return { width: 800, height: 1120 };
    }
  };
  ...
  const handleSubmitEdit = async () => {
    try {
      ...
      const { width: refW, height: refH } = getReferenceDimensions();

      const texts = allTextElements.map((el) => ({
        id: el.id,
        ...
        textAlign: el.alignment || el.selectedAlignment || "left",
        isBold: !!el.isBold,
        isItalic: !!el.isItalic,
        isUnderline: !!el.isUnderline,
      }));

      const images = allImageElements.map((el) => ({
        id: el.id,
        pageNumber: el.pageNumber,
        x: el.x,
        y: el.y,
      ...
      const drawings = allDrawElements.map((el) => ({
        id: el.id,
        pageNumber: el.pageNumber,
        type: el.type,
        color: el.color,
      ...
      const edits = { texts, images, drawings };

      const formData = new FormData();
      formData.append("file", currentFile.file);
      formData.append("edits", JSON.stringify(edits));

      const response = await Api.post("/tools/edit-pdf", formData, {
      ...
      if (payload?.fileUrl && payload?.dbTaskId) {
        const encodedPath = encodeURIComponent(payload.fileUrl);
        router.push(`/downloads/document=${encodedPath}?dbTaskId=${payload.dbTaskId}&tool-type=edit-pdf`);
      } else {
  ...
  const reorderElements = (draggedId, targetId, draggedType, targetType) => {
    ...
    const draggedIndex = combinedElements.findIndex((el) => el.id === draggedId);
    ...
    const pageElements = combinedElements.filter((el) => el.pageNumber === draggedElement.pageNumber);
    const draggedPageIndex = pageElements.findIndex((el) => el.id === draggedId);
    ...
    const updatedTextElements = allTextElements.map((textEl) => {
      if (textEl.pageNumber === draggedElement.pageNumber) {
        const reorderedEl = reorderedPageElements.find((el) => el.id === textEl.id);
    ...
    const updatedImageElements = allImageElements.map((imageEl) => {
      if (imageEl.pageNumber === draggedElement.pageNumber) {
        const reorderedEl = reorderedPageElements.find((el) => el.id === imageEl.id);
    ...
    const updatedDrawElements = allDrawElements.map((drawEl) => {
      if (drawEl.pageNumber === draggedElement.pageNumber) {
        const reorderedEl = reorderedPageElements.find((el) => el.id === drawEl.id);
  ...
  const handleDrop = (e, targetElement, targetType) => {
    ...
    reorderElements(draggedElement.id, targetElement.id, draggedElement.type, targetType);
  ...
  const renderElementItem = (element, index, pageElements) => {
    ...
    const isImage = element.src !== undefined && element.text === undefined && element.type === undefined;
    ...
    const canDrop = draggedElement && draggedElement.pageNumber === element.pageNumber;

    return (
      <div
        key={element.id}
        draggable
        onDragStart={(e) => handleDragStart(e, element, isText ? "text" : isDraw ? "draw" : "image")}
        onDragOver={(e) => handleDragOver(e, element, isText ? "text" : isDraw ? "draw" : "image")}
        onDragLeave={handleDragLeave}
        onDrop={(e) => handleDrop(e, element, isText ? "text" : isDraw ? "draw" : "image")}
        ...
        ${isDragOver && canDrop ? "ring-2 ring-red-400 bg-red-100 transform scale-[1.02]" : ""}
        ${draggedElement && !canDrop && draggedElement.pageNumber !== element.pageNumber ? "opacity-60" : ""}
      `}
      >
        {isDragOver && canDrop && (
            ...
            {isText ? (
              ...
              element.type === "brush" ? (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="m9.06 11.9 8.07-8.06a2.85 2.85 0 1 1 4.03 4.03l-8.06 8.08" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7.07 14.94c-1.66 0-3 1.35-3 3.02 0 1.33-2.5 1.52-2 2.02 1.08-2 2.5-4 4-4z" />
                </svg>
              ) : element.type === "line" ? (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 17L17 7" />
                  <polyline points="7,7 17,7 17,17" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
                </svg>
              ) : element.type === "square" ? (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <rect x="3" y="3" width="18" height="18" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
                </svg>
              ) : element.type === "circle" ? (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2L22 20H2L12 2Z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
                </svg>
              ) : (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <img src={element.src} alt="Preview" className="w-full h-full object-cover rounded" />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900">
              {isText
                ...
                : isEmoji
                  ...
                  : isDraw
                    ? `${element.type?.charAt(0).toUpperCase() + element.type?.slice(1) || "Drawing"} ${index + 1}`
                    : `Image ${index + 1}`}
            </p>
            <div className="flex items-center gap-2 text-xs text-gray-500">
              {isText ? (
                ...
                <>
                  <div className="w-3 h-3 rounded-full border border-gray-300" style={{ backgroundColor: element.color || "#000000" }} />
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            title={`Edit ${isText ? "text" : isEmoji ? "emoji" : isDraw ? "drawing" : "image"}`}
          >
          ...
          <button
            onClick={(e) => {
              ...
              if (isText) {
                setAllTextElements((prev) => prev.filter((t) => t.id !== element.id));
                setDeleteSpecificElement({ textId: element.id, pageNumber: element.pageNumber });
              } else if (isDraw) {
                setAllDrawElements((prev) => prev.filter((draw) => draw.id !== element.id));
                setDeleteSpecificDrawElement({ drawId: element.id, pageNumber: element.pageNumber });
              } else {
                setAllImageElements((prev) => prev.filter((img) => img.id !== element.id));
                setDeleteSpecificImageElement({ imageId: element.id, pageNumber: element.pageNumber });
              }
            }}
            className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded transition-colors"
            title={`Delete ${isText ? "text" : isEmoji ? "emoji" : isDraw ? "drawing" : "image"}`}
          >
  ...
  const handleDrawUpdate = (drawingId, updates) => {
    setAllDrawElements((prev) =>
      prev.map((drawing) => (drawing.id === drawingId ? { ...drawing, ...updates } : drawing))
    );
  };

  const handleDrawDelete = (drawingId) => {
    setAllDrawElements((prev) => prev.filter((drawing) => drawing.id !== drawingId));
  };
  ...
  return isUploading ? (
    <ProgressScreen uploadProgress={uploadProgress} />
  ) : (
    <div className="h-full">
      <div className="grid grid-cols-1 md:grid-cols-7 border w-full h-full">
        {/* Thumbnails Sidebar - Left */}
        {isThumbnailVisible && (
          <div className="hidden md:h-[calc(100vh-82px)] md:flex justify-center items-start md:col-span-1 lg:col-span-1 bg-gray-50 border-r overflow-y-auto custom-scrollbar">
        ...
        {/* Main Content Area */}
        <div
          className={`${isThumbnailVisible ? "md:col-span-4 lg:col-span-4" : "md:col-span-5"
        ...
        >
          ...
          <div className="w-full">
            <div className="flex items-center gap-3 p-2 pl-10 bg-white border-b border-gray-20 relative">
              <button
                className={`p-2 rounded transition-colors ${selectedTool === "pan"
              ...
              <button
                className={`p-3 rounded-lg transition-all duration-200 ${textEditingState.showTextToolbar
                    ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                    : "hover:bg-red-100 text-gray-700"
                  }`}
              ...
              <button
                className={`p-3 rounded-lg transition-all duration-200 ${imageEditingState.showImageToolbar
                    ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                    : "hover:bg-red-100 text-gray-700"
                  }`}
              ...
              <button
                className={`p-3 rounded-lg transition-all duration-200 ${drawEditingState.showDrawToolbar
                    ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                    : "hover:bg-red-100 text-gray-700"
                  }`}
            ...
            {textEditingState.showTextToolbar && (
              <div className="absolute z-50 flex items-center justify-center gap-2 p-2 ml-[15px] bg-gray-50 border-b border-gray-200">
                ...
                <button
                  onClick={handleBold}
                  className={`p-2 rounded transition-colors ${textEditingState.isBold ? "bg-red-100 text-red-600" : "hover:bg-gray-200"
                    }`}
                >
                ...
                <button
                  ...
                  className={`p-2 rounded transition-colors ${textEditingState.isItalic ? "bg-red-100 text-red-600" : "hover:bg-gray-200"
                    }`}
                >
                ...
                <button
                  ...
                  className={`p-2 rounded transition-colors ${textEditingState.isUnderline ? "bg-red-100 text-red-600" : "hover:bg-gray-200"
                    }`}
                >
                  ...
                  <div className="flex items-center p-1 border border-gray-300 rounded hover:bg-gray-50">
                    <div className="w-6 h-4 flex flex-col">
                      ...
                      <div
                        ...
                        style={{ backgroundColor: textEditingState.selectedColor }}
                      ></div>
                    </div>
                    <input
                      type="color"
                  ...
                  <div className="flex items-center p-1 border border-gray-300 rounded hover:bg-gray-50">
                    <div className="w-6 h-4 flex flex-col relative">
                      <svg className="w-4 h-3" viewBox="0 0 16 12" fill="none">
                        <path d="M2 2h12v8H2z" fill="white" stroke="black" strokeWidth="1" />
                        <path d="M4 4h8v4H4z" fill="currentColor" />
                      </svg>
                      <div
                        className={`w-6 h-1 rounded-b`}
                        style={{ backgroundColor: textEditingState.selectedBgColor }}
                      ></div>
                    </div>
                    <input
                      type="color"
                  ...
                  <div className="flex items-center border border-gray-300 rounded">
                    <button
                      onClick={() =>
                        setTextEditingState((prev) => ({
                          ...prev,
                          isAlignmentDropdownOpen: !prev.isAlignmentDropdownOpen,
                        }))
                      }
                    ...
                    <div className="absolute mt-1 w-28 bg-white border border-gray-200 rounded shadow-md z-10">
                      {options.map((opt) => (
                        <button
                          ...
                          className={`w-full flex items-center gap-2 p-1 text-sm hover:bg-gray-100 ${textEditingState.selectedAlignment === opt.value
                ...
                <div className="flex items-center gap-2 bg-white border border-gray-300 rounded px-2 py-1">
                  <input
                    ...
                    style={{
                      background: `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${textEditingState.opacity * 100
                        }%, #e5e7eb ${textEditingState.opacity * 100
                        }%, #e5e7eb 100%)`,
                    }}
                  />
            ...
            {imageEditingState.showImageToolbar && (
              <div className="absolute z-50 flex items-center justify-center gap-2 p-2 ml-[15px] bg-gray-50 border-b border-gray-200 rounded-lg shadow-lg">
                    ...
                    <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded shadow-lg z-10 min-w-[80px]">
                      {[25, 50, 75, 100].map((option) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-100 ${imageEditingState.opacity === option
                ...
                <button
                  ...
                  className={`p-2 hover:bg-gray-200 rounded ${imageEditingState.flipHorizontal ? "bg-red-100 text-red-600" : ""
                    }`}
                ...
                <button
                  ...
                  className={`p-2 hover:bg-gray-200 rounded ${imageEditingState.flipVertical ? "bg-red-100 text-red-600" : ""
                    }`}
                  title="Flip Vertical"
                >
            ...
            {drawEditingState.showDrawToolbar && (
              <div className="absolute z-50 flex items-center justify-center gap-2 p-2 ml-[15px] bg-gray-50 border-b border-gray-200 rounded-lg shadow-lg">
                ...
                <button
                  className={`p-2 rounded transition-colors ${drawEditingState.selectedTool === "line"
                      ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                      : "hover:bg-gray-200"
                    }`}
                  onClick={() =>
                ...
                <button
                  className={`p-2 rounded transition-colors ${drawEditingState.selectedTool === "brush"
                      ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                      : "hover:bg-gray-200"
                    }`}
                  onClick={() =>
                ...
                <button
                  className={`p-2 rounded transition-colors ${drawEditingState.selectedTool === "arrow"
                      ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                      : "hover:bg-gray-200"
                    }`}
                  onClick={() =>
                ...
                <button
                  className={`p-2 rounded transition-colors ${drawEditingState.selectedTool === "square"
                      ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                      : "hover:bg-gray-200"
                    }`}
                  onClick={() =>
                ...
                <button
                  className={`p-2 rounded transition-colors ${drawEditingState.selectedTool === "circle"
                      ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                      : "hover:bg-gray-200"
                    }`}
                  onClick={() =>
                ...
                <button
                  className={`p-2 rounded transition-colors ${drawEditingState.selectedTool === "triangle"
                      ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                      : "hover:bg-gray-200"
                    }`}
                  onClick={() =>
                ...
                <div className="relative">
                  <button
                    className={`p-2 rounded transition-colors ${drawEditingState.selectedTool === "emoji"
                        ? "bg-red-100 text-red-600 ring-2 ring-red-400"
                        : "hover:bg-gray-200"
                      }`}
                    onClick={() =>
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    if (allDrawElements.length > 0) {
                      const lastElement = allDrawElements[allDrawElements.length - 1];
            ...
            >
              ...
              >
                {files.map((file) => {
                  ...
                  return (
                    <div key={file.id} className="w-full flex justify-center py-4">
                      <div className="w-full max-w-3xl">
                        <Allpagespreview
                          ...
                          clearAllTextElements={clearAllTextElements}
                          onClearAllComplete={() => setClearAllTextElements(false)}
                          deleteSpecificElement={deleteSpecificElement}
                          onDeleteSpecificComplete={() => setDeleteSpecificElement(null)}
                          imageEditingState={imageEditingState}
                          onImageAdd={handleImageAdd}
                          onImageUpdate={handleImageUpdate}
                          onImageDelete={handleImageDelete}
                          clearAllImageElements={clearAllImageElements}
                          onClearAllImageComplete={() => setClearAllImageElements(false)}
                          deleteSpecificImageElement={deleteSpecificImageElement}
                          onDeleteSpecificImageComplete={() => setDeleteSpecificImageElement(null)}
                          drawEditingState={drawEditingState}
                          onDrawAdd={handleDrawAdd}
                          onDrawUpdate={handleDrawUpdate}
                          onDrawDelete={handleDrawDelete}
                          clearAllDrawElements={clearAllDrawElements}
                          onClearAllDrawComplete={() => setClearAllDrawElements(false)}
                          deleteSpecificDrawElement={deleteSpecificDrawElement}
                          onDeleteSpecificDrawComplete={() => setDeleteSpecificDrawElement(null)}
                          combinedElements={getCombinedElements()}
                          allTextElements={allTextElements}
                          allImageElements={allImageElements}
                          allDrawElements={allDrawElements}
        ...
        {/* Right Sidebar - Elements List with improved design */}
        <div className="hidden md:flex md:col-span-2 flex-col bg-white border-l h-[calc(100vh-82px)] overflow-hidden">
          <div className="flex flex-col h-full">
            {/* Header */}
            <div className="flex-shrink-0 border-b border-gray-100">
              <h3 className="text-2xl font-bold text-gray-900 text-center py-4 bg-white">
                Edit PDF
              </h3>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              <div className="p-4">
                ...
                {/* Unhealthy Files Warning */}
                {hasUnhealthyFiles && (
                  <div className="bg-yellow-50 rounded-xl p-4 mb-6">
                    <p className="text-sm text-yellow-800">
                      File has preview issues but can still be edited. Check the highlighted areas.
                    </p>
                  </div>
                )}
                ...
                {passwordProtectedFiles.size > 0 && (
                  <div className="bg-yellow-50 rounded-xl p-4 mb-6">
                    <p className="text-sm text-yellow-800">
                      Password-protected file detected. Password may be required for editing.
                    </p>
                  </div>
                )}
                        ...
                        <div key={`page-${pageNumber}`}>
                          <div className="flex items-center justify-between mb-3 border-b border-gray-200 pb-2">
                            ...
                            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
                              {pageElements.length} element{pageElements.length !== 1 ? "s" : ""}
                            </span>
                          </div>
              ...
              {!limitsExceeded.hasAnyExceeded ? (
                (allTextElements.length > 0 ||
                  ...
                  <button
                    ...
                    disabled={isUploading || !currentFile?.file}
                    className={`w-full py-3 px-6 rounded-xl font-semibold text-white transition-all duration-200 flex items-center justify-center gap-2 ${!isUploading && currentFile?.file
                        ? "bg-red-600 hover:bg-red-700 hover:scale-105 shadow-lg"
                        : "bg-gray-300 cursor-not-allowed"
                      }`}
                  >
                    Edit PDF <ArrowRight className="w-5 h-5" />
                  </button>
                )
              ) : (
                <LimitsExceeded
              ...
              {(allTextElements.length === 0 &&
                ...
                allDrawElements.length === 0) && (
                  <p className="text-xs text-gray-500 text-center mt-2">
                    Add elements to your PDF to get started
                  </p>
                )}
            </div>
          </div>
...
//                 <button onClick={handleSubmitEdit} disabled={isUploading || !currentFile?.file} className="w-full bg-red-500 hover:bg-red-600 disabled:bg-red-300 text-white py-3 px-4 rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
//                   <span>Edit PDF</span>
