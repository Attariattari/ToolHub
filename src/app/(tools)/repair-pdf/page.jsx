...
const FileInfoSection = ({ files, totalSize, totalPages, passwordProtectedCount }) => (
  <div className="mb-6">
...
const Sidebar = ({
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 flex-1">
      <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Repair PDF</h3>

      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Repair your corrupted or damaged PDF files. Our tool will attempt to recover as much data as possible from your PDF documents.
        </p>
      </div>
      ...
      {hasUnhealthyFiles && (
        <div className="bg-yellow-50 rounded-xl p-4 mb-6">
          <p className="text-sm text-yellow-800">
            Some files have preview issues but can still be repaired. Check the yellow-highlighted files.
          </p>
        </div>
      )}
      ...
      {passwordProtectedCount > 0 && (
        <div className="bg-green-50 rounded-xl p-4 mb-6">
          <p className="text-sm text-green-800">
            {passwordProtectedCount} password-protected file{passwordProtectedCount > 1 ? "s" : ""} detected. Passwords will be required for repair.
          </p>
        </div>
      )}

      {files.length > 0 && <FileInfoSection files={files} totalSize={totalSize} totalPages={totalPages} passwordProtectedCount={passwordProtectedCount} />}
    </div>
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      ...
      {files.length === 0 && <p className="text-xs text-gray-500 text-center mt-2">Select PDF files to repair</p>}
    </div>
  </div>
)
...
export default function RepairPDFPage() {
  ...
  const handlePasswordSubmit = useCallback(async (passwords) => {
    ...
    try {
      ...
      const response = await Api.post("/tools/repair-pdf", formData, {
      ...
      if (response.data) {
        ...
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=repair-pdf`
        router.push(downloadUrl)
      } else {
        toast.error("No repaired files received from server")
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || "Error repairing files")
    } finally {
  ...
  if (files.length === 0) {
    return (
      <SafeFileUploader
        ...
        pageTitle="Repair PDF"
        pageSubTitle="Upload a corrupted or damaged PDF and we will try to fix it. Depending on the damage, we may be able to recover it partially or completely."
  ...
  const sidebarProps = {
    files, totalSize, totalPages, hasUnhealthyFiles, passwordProtectedCount, limitsExceeded, onRepair: handleRepair
  }
          ...
          <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => setShowMobileSidebar(false)}>
            <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-xl overflow-y-auto custom-scrollbar pb-16" onClick={(e) => e.stopPropagation()}>
              <div className="p-4 border-b flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-900">Repair PDF</h3>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                {!limitsExceeded.hasAnyExceeded ? (
                  ...
                  >
                    Repair PDF <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
...
//                 Repair your corrupted or damaged PDF files. Our tool will attempt to recover as much data as possible from your PDF.
//               </p>
//             </div>
...
//               <div className="bg-yellow-50 rounded-xl p-4 mb-6">
//                 <p className="text-sm text-yellow-800">
//                   {passwordProtectedFiles.size} password-protected file{passwordProtectedFiles.size > 1 ? "s" : ""} detected. Passwords will be required for repair.
//                 </p>
//               </div>
//             )}
//           </div>
...
//             {files.length === 0 && (
//               <p className="text-xs text-gray-500 text-center mt-2">Select PDF files to repair</p>
//             )}
//           </div>
//         </div>
//       </div>
