...
const Sidebar = ({
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 flex-1">
      <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">PDF to Word Converter</h3>

      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Convert your PDF files to Word documents. Each page will be converted with high quality preservation and editable text.
        </p>
      </div>
...
export default function PDFToWordPage() {
  ...
  const handlePasswordSubmit = useCallback(async (passwords) => {
    ...
    try {
      ...
      if (response.data) {
        const encodedPath = encodeURIComponent(response.data.data.convertedFiles)
        const downloadUrl = `/downloads/document=${encodedPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=pdf-to-word`
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                {!limitsExceeded.hasAnyExceeded ? (
                  ...
                  >
                    Convert to Word <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
...
//         if (response.data) {
//           const downloadUrl = `/downloads/document=${response.data.data.convertedFiles}?dbTaskId=${response.data.data.dbTaskId}&tool-type=pdf-to-word`
//           router.push(downloadUrl)
//         } else {
//           toast.error("No converted files received from server")
//         }
