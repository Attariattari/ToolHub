...
const Sidebar = ({
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 flex-1">
      <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Word to PDF Converter</h3>

      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Convert your Word documents (.doc, .docx) to PDF format. All formatting, images, and layouts will be preserved in the converted PDF files.
        </p>
      </div>
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      ...
      {files.length === 0 && <p className="text-xs text-gray-500 text-center mt-2">Select Word files to convert</p>}
    </div>
  </div>
)
...
export default function WordToPDFPage() {
  ...
  const handleFiles = useCallback(async (newFiles) => {
    const fileObjects = await Promise.all(
      newFiles.map(async (file, index) => {
        ...
        // Set health check to true for Word files
        setFileHealthCheck(prev => ({ ...prev, [id]: true }))
  ...
  const removeFile = useCallback((id) => {
    cleanupFile(id)
    setFileHealthCheck(prev => { const { [id]: removed, ...rest } = prev; return rest })
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=word-to-pdf`
  ...
  const hasUnhealthyFiles = useMemo(() => Object.values(fileHealthCheck).some(health => health === false), [fileHealthCheck])
  ...
  if (files.length === 0) {
    return (
      <SafeFileUploader
        ...
        pageSubTitle="Convert your Word documents to PDF with perfect formatting preservation."
  ...
  const sidebarProps = {
    files, totalSize, hasUnhealthyFiles, limitsExceeded, onConvert: handleConvert
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-4">
            {files.map(file => (
              <WordPreview
                ...
                file={file}
                onRemove={removeFile}
                isHealthy={fileHealthCheck[file.id] !== false}
              />
            ))}
          </div>
        </div>
