...
import { FileText, X, ArrowRight, Plus, Move, Check, Settings, AlertTriangle } from "lucide-react"
...
const PDFPreview = memo(({ file, pageNumber = 1, width = 120, className = "" }) => {
  ...
  const renderPreview = () => {
    if (!shouldRender) {
      return <div className="w-full h-full bg-gray-50 flex items-center justify-center rounded-lg" style={{ height: `${width * 1.4}px` }} />
    }

    if (hasError) {
      return (
        <div className="w-full h-full bg-blue-50 flex flex-col items-center justify-center rounded-lg p-2">
          <FileText className="w-8 h-8 text-blue-400 mb-1" />
          <div className="text-xs text-blue-600 font-medium text-center">Could not load</div>
        </div>
      )
    }
...
const FileInfoSection = ({ file, pdfPages, fileSizeInMB }) => (
  <div className="mb-6">
    ...
    <div className="space-y-2 text-sm">
      {[
        ...
        ["Total pages:", pdfPages || "N/A"],
        ["Format:", "PDF"]
      ].map(([label, value]) => (
        <div key={label} className="flex items-center justify-between">
          <span className="text-blue-700">{label}</span>
          <span className="font-semibold text-blue-900 text-right flex-1 ml-2 truncate" title={typeof value === 'string' ? value : undefined}>
            {value}
          </span>
        </div>
      ))}
...
const LimitsExceeded = ({ limitsExceeded, pdfPages, fileSizeInMB }) => (
  <div className="bg-white rounded-xl border-2 border-blue-200 p-4">
    <h4 className="font-semibold text-blue-600 mb-3 text-center">Limits Exceeded</h4>
    <div className="space-y-2 text-sm">
      {limitsExceeded.exceedsPages && (
        <div className="flex items-center justify-between">
          <span className="text-gray-600">Pages:</span>
          <span className="font-semibold text-blue-600">{pdfPages} / {LIMITS.MAX_PAGES}</span>
        </div>
      )}
      {limitsExceeded.exceedsSize && (
        <div className="flex items-center justify-between">
          <span className="text-gray-600">File size:</span>
          <span className="font-semibold text-blue-600">{fileSizeInMB} / {LIMITS.MAX_SIZE_MB} MB</span>
        </div>
      )}
    </div>
    <p className="text-xs text-blue-600 text-center mt-3">Please use a smaller file to continue.</p>
  </div>
)

// Password Protected Notice Component
const PasswordProtectedNotice = () => (
  <div className="bg-yellow-50 rounded-xl p-4 mb-6 border border-yellow-200">
    <div className="flex items-start gap-3">
      <AlertTriangle className="w-5 h-5 text-yellow-600 flex-shrink-0 mt-0.5" />
      <div>
        <h4 className="font-semibold text-yellow-800 mb-2">Password Protected File</h4>
        <p className="text-sm text-yellow-700 mb-3">
          This file appears to be password protected. Processing cannot continue with encrypted files.
        </p>
        <a
          href="/unlock-pdf"
          className="inline-flex items-center gap-2 text-sm text-yellow-800 hover:text-yellow-900 underline font-medium"
        >
          Unlock PDF first <ArrowRight className="w-3 h-3" />
        </a>
      </div>
    </div>
  </div>
...
const Sidebar = ({
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 border-b">
      <h3 className="text-2xl font-bold text-gray-900 mb-4 text-center">Split PDF</h3>
    ...
    <div className="flex-1 p-6">
      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Split your PDF into smaller files based on page ranges, specific pages, or file size. Choose your preferred method below.
        </p>
      </div>

      {isPasswordProtected && <PasswordProtectedNotice />}

      {/* {file && <FileInfoSection file={file} pdfPages={pdfPages} fileSizeInMB={fileSizeInMB} />} */}

      {/* Tab Content */}
      {activeTab === "range" && !isPasswordProtected && (
        <div className="space-y-6">
          ...
          {rangeMode === "custom" && (
            <div className="space-y-4 pb-4">
              ...
              <button
                onClick={addRange}
                className="w-full p-2 border-2 border-dashed border-blue-300 text-blue-600 rounded-lg hover:border-blue-400 hover:text-blue-700 transition-colors duration-200 flex items-center justify-center gap-2 text-sm"
              >
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-4 h-4 bg-gray-100 border-blue-300 rounded focus:ring-blue-500 text-blue-600"
                />
      ...
      {activeTab === "pages" && !isPasswordProtected && (
        <div className="space-y-6">
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 p-2 bg-blue-100 text-blue-800 rounded-lg hover:bg-blue-200 transition-colors duration-200 text-sm"
                >
                  Clear All
                </button>
              </div>
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  Selected pages will be converted into separate PDF files. {selectedPages.size} PDF will be created.
                </p>
              </div>
            </div>
          )}
      ...
      {activeTab === "size" && !isPasswordProtected && (
        <div className="space-y-6">
    ...
    {/* Split Button */}
    <div className="max-md:hidden flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      {!limitsExceeded.hasAnyExceeded && !isPasswordProtected ? (
        <button
          onClick={onSplit}
          disabled={
            (activeTab === "pages" && extractMode === "select" && selectedPages.size === 0)
          }
          className={`w-full py-3 px-6 rounded-xl font-semibold text-white transition-all duration-200 flex items-center justify-center gap-2 ${(activeTab === "pages" && extractMode === "select" && selectedPages.size === 0)
        ...
        <LimitsExceeded limitsExceeded={limitsExceeded} pdfPages={pdfPages} fileSizeInMB={fileSizeInMB} />
      )}
...
export default function SplitPDFPage() {
  ...
  const limitsExceeded = useMemo(() => {
    const exceedsPages = pdfPages > LIMITS.MAX_PAGES
    const exceedsSize = fileSizeInMB > LIMITS.MAX_SIZE_MB
    return { exceedsPages, exceedsSize, hasAnyExceeded: exceedsPages || exceedsSize }
  }, [pdfPages, fileSizeInMB])
  ...
  const handleSizeInputChange = useCallback((value) => {
  ...
  const handleSizeUnitChange = useCallback((newUnit) => {
  ...
  const updateRange = useCallback((id, field, value) => {
  ...
  const handleExtractModeChange = useCallback((mode) => {
    setExtractMode(mode)
    if (mode === "extract-all") {
      selectAllPages()
    } else {
  ...
  }, [rangeMode, ranges, file, dragOverRangeIndex, draggedRangeIndex, handleRangeDragStart, handleRangeDragOver, handleRangeDrop])

  // Memoized content renderers
  const rangeContent = useMemo(() => {
    if (rangeMode === "custom") {
      return customRangeCards || (
  ...
  }, [rangeMode, customRangeCards, file, pdfPages, fixedRanges, dragOverRangeIndex, draggedRangeIndex, handleRangeDragStart, handleRangeDragOver, handleRangeDrop])
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {pageNumbers.map((pageNum) => (
          <div
            key={pageNum}
            className={`bg-white rounded-xl border-2 p-3 cursor-pointer transition-all duration-200 ${selectedPages.has(pageNum) ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"
              }`}
  ...
  const renderMainContent = useCallback(() => {
    switch (activeTab) {
      case "range": return rangeContent
      case "pages": return pagesContent
      case "size": return sizeContent
      default: return rangeContent
    }
  ...
  }, [file, activeTab, rangeMode, fixedRanges, ranges, mergeRanges, fixedRangeSize, extractMode, selectedPages, mergeExtracted, maxSize, sizeUnit, allowCompression, router])
              ...
              <div className="flex-1">
                {!limitsExceeded.hasAnyExceeded && !isPasswordProtected ? (
                  ...
                  <div className="bg-yellow-50 rounded-xl p-3 text-center border border-yellow-200">
                    <p className="text-sm text-yellow-700 font-medium">Password Protected!</p>
                    <a
                      href="/unlock-pdf"
                  ...
                  <div className="bg-blue-50 rounded-xl p-3 text-center border border-blue-200">
                    <p className="text-sm text-blue-600 font-medium">Limits exceeded!</p>
                    <p className="text-xs text-blue-500 mt-1">Use smaller file to continue</p>
                  </div>
                )}
              </div>
              <button
