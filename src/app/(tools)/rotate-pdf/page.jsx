...
import { FileText, X, ArrowRight, RotateCw, RotateCcw, RefreshCw, Settings } from "lucide-react"
...
const PDFPreview = memo(({ file, rotation, isLoading, onLoadSuccess, onLoadError, onRotate, onRemove, isHealthy, isPasswordProtected }) => {
        ...
        <div className="w-full h-full relative flex justify-center items-center bg-gray-100 rounded-lg">
          {!isLoading ? (
            <Document
              ...
              loading={<div className="flex items-center justify-center"><RefreshCw className="w-8 h-8 text-gray-400 animate-spin" /></div>}
              error={
      ...
      <div className="relative h-56 p-3 pt-10">
        ...
        <div className="absolute top-1 right-2 flex gap-1 z-30">
          <button
            ...
            title={isPasswordProtected ? "Rotate file (password required for processing)" : isHealthy ? "Rotate file" : "Cannot rotate - PDF issue"}
          >
...
// Components
const FileInfoSection = ({ files, totalSize, hasRotations, fileRotations, passwordProtectedFiles, totalPages }) => (
  <div className="mb-6">
    ...
    <div className="space-y-2 text-sm">
      {[
        ...
        ...(hasRotations ? [["Files with rotation:", Object.values(fileRotations).filter(r => r > 0).length]] : []),
        ...(passwordProtectedFiles.size > 0 ? [["Password protected:", passwordProtectedFiles.size]] : [])
      ].map(([label, value]) => (
        <div key={label} className="flex items-center justify-between">
          <span className="text-blue-700">{label}</span>
          <span className={`font-semibold ${label === "Password protected:" ? "text-yellow-600" : "text-blue-900"}`}>{value}</span>
        </div>
      ))}
    </div>
  </div>
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 flex-1">
      ...
      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Rotate your PDFs the way you need them. You can rotate multiple PDFs at once and set different rotations for each file.
        </p>
      </div>
      ...
      {files.length > 1 && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-lg font-semibold text-blue-900">Batch Rotation:</h4>
            <button onClick={onResetRotations} className="text-red-600 hover:text-red-800 text-sm font-medium">
              Reset all
            </button>
          </div>
          <div className="space-y-3">
            <button
              ...
              className="w-full p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200 flex items-center justify-center gap-2 font-medium text-sm"
            >
      ...
      {passwordProtectedFiles.size > 0 && (
        <div className="bg-yellow-50 rounded-xl p-4 mb-6">
          <p className="text-sm text-yellow-800">
            {passwordProtectedFiles.size} password-protected file{passwordProtectedFiles.size > 1 ? "s" : ""} detected.
            You can rotate them, but passwords will be required for processing.
          </p>
        </div>
      )}
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      {!limitsExceeded.hasAnyExceeded ? (
        ...
        >
          Rotate PDF <ArrowRight className="w-5 h-5" />
        </button>
      ) : (
...
export default function RotatePDFPage() {
  ...
  const handlePasswordSubmit = useCallback(async (passwords) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=rotate-pdf`
  ...
  const sidebarProps = {
    ...
    fileRotations,
    limitsExceeded,
    onRotateRight: rotateAllRight,
    onRotateLeft: rotateAllLeft,
    onResetRotations: resetAllRotations,
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                {!limitsExceeded.hasAnyExceeded ? (
                  ...
                  >
                    Rotate PDF <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
