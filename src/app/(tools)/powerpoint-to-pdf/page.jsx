...
import { Presentation, X, ArrowRight, Settings } from "lucide-react"
...
// Components
const PowerPointPreview = memo(({ file, onRemove }) => {
...
const Sidebar = ({ files, totalSize, limitsExceeded, onConvert }) => (
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      ...
      {files.length === 0 && <p className="text-xs text-gray-500 text-center mt-2">Select PowerPoint files to convert</p>}
    </div>
  </div>
)
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=powerpoint-to-pdf`
