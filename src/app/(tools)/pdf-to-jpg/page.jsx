...
import { FileText, X, ArrowRight, ImageIcon, Download, Settings } from "lucide-react"
...
const QUALITY_OPTIONS = [
  { value: "normal", label: "Normal", subtitle: "Recommended" },
  { value: "high", label: "High", subtitle: "Larger files" }
]
  ...
  <div className="w-full h-full relative flex justify-center items-center bg-gray-100 rounded-lg">
    {isLoading ? (
      ...
      <Document
        ...
        loading={<div className="w-8 h-8 border-4 border-gray-300 border-t-red-600 rounded-full animate-spin" />}
        error={
...
const ConversionModeOption = ({ mode, isSelected, onSelect, estimatedCount }) => {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className={`font-semibold ${mode.isBlue ? 'text-blue-600' : 'text-gray-900'}`}>{mode.title}</h4>
          </div>
          <p className="text-sm text-gray-600">
            {mode.description} {mode.value === "pages" && `${estimatedCount} JPG will be created.`}
          </p>
        </div>
      </div>
    </div>
...
const QualitySelector = ({ quality, onQualityChange }) => (
  <div className="mb-6">
    <h4 className="font-semibold text-gray-900 mb-3">Image quality</h4>
    <div className="flex gap-2">
      {QUALITY_OPTIONS.map(option => (
        <button
          key={option.value}
          onClick={() => onQualityChange(option.value)}
          className={`flex-1 py-3 px-4 rounded-xl border-2 font-medium transition-all duration-200 ${quality === option.value
        ...
        >
          <div className="text-center">
            ...
            <div className="text-xs mt-1">{option.subtitle}</div>
          </div>
        </button>
      ))}
    </div>
...
const Sidebar = ({
  ...
  <div className="flex-1 overflow-y-auto custom-scrollbar">
    <div className="p-6">
      <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">PDF to JPG options</h3>

      <div className="space-y-4 mb-6">
        {CONVERSION_MODES.map(mode => (
          <ConversionModeOption
            key={mode.value}
            mode={mode}
            isSelected={conversionMode === mode.value}
            onSelect={setConversionMode}
            estimatedCount={estimatedJPGCount}
          />
        ))}
      </div>

      <QualitySelector quality={imageQuality} onQualityChange={setImageQuality} />
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      {!limitsExceeded.hasAnyExceeded ? (
        ...
        >
          Convert to JPG <ArrowRight className="w-5 h-5" />
        </button>
      ) : (
...
export default function PDFToJPGPage() {
  ...
  const handlePasswordSubmit = useCallback(async (passwords) => {
    ...
    try {
      ...
      if (response.data) {
        const encodedZipPath = encodeURIComponent(response.data.data.zipFileUrl)
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=pdf-to-jpg`
  ...
  const estimatedJPGCount = useMemo(() => conversionMode === "pages" ? totalPages : Math.ceil(totalPages * 0.5), [conversionMode, totalPages])
  ...
  const sidebarProps = {
    conversionMode, setConversionMode, imageQuality, setImageQuality,
    files, totalSize, totalPages, estimatedJPGCount, hasUnhealthyFiles,
          ...
          <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => setShowMobileSidebar(false)}>
            <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-xl overflow-y-auto custom-scrollbar pb-16" onClick={(e) => e.stopPropagation()}>
              <div className="p-4 border-b flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-900">PDF to JPG options</h3>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                {!limitsExceeded.hasAnyExceeded ? (
                  ...
                  >
                    Convert to JPG <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
...
//   // Conversion options
//   const [conversionMode, setConversionMode] = useState("pages") // "pages" or "images"
//   const [imageQuality, setImageQuality] = useState("normal") // "normal" or "high"
...
//       return totalPages
//     } else {
//       return Math.ceil(totalPages * 0.5)
//     }
//   }, [conversionMode, totalPages])

//   const limitsExceeded = useMemo(() => {
//     const exceedsFiles = files.length > MAX_FILES
//     const exceedsSize = Number.parseFloat(totalSize) > MAX_SIZE_MB
//     const exceedsPages = totalPages > MAX_PAGES
//     return { exceedsFiles, exceedsSize, exceedsPages, hasAnyExceeded: exceedsFiles || exceedsSize || exceedsPages }
//   }, [files.length, totalSize, totalPages])
...
//                 {/* Page to JPG Option */}
//                 <div
//                   className={`border-2 rounded-xl p-4 cursor-pointer transition-all duration-200 ${conversionMode === "pages" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"
//                     }`}
...
//                     <div className="flex-shrink-0 mt-1">
//                       <div
//                         className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${conversionMode === "pages" ? "border-blue-500 bg-blue-500" : "border-gray-300"
//                           }`}
//                       >
...
//                         <h4 className="font-semibold text-gray-900">PAGE TO JPG</h4>
//                       </div>
//                       <p className="text-sm text-gray-600">
//                         Every page of this PDF will be converted into a JPG file. {estimatedJPGCount} JPG will be
//                         created.
//                       </p>
//                     </div>
//                   </div>
...
//                 <div
//                   className={`border-2 rounded-xl p-4 cursor-pointer transition-all duration-200 ${conversionMode === "images" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"
//                     }`}
//                   onClick={() => setConversionMode("images")}
//                 >
//                   <div className="flex items-start gap-3">
//                     <div className="flex-shrink-0 mt-1">
//                       <div
//                         className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${conversionMode === "images" ? "border-blue-500 bg-blue-500" : "border-gray-300"
//                           }`}
//                       >
//                         {conversionMode === "images" && (
//                           <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
//                             <path
//                               fillRule="evenodd"
//                               d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
...
//                         <h4 className="font-semibold text-blue-600">EXTRACT IMAGES</h4>
//                       </div>
//                       <p className="text-sm text-gray-600">
...
//                     className={`flex-1 py-3 px-4 rounded-xl border-2 font-medium transition-all duration-200 ${imageQuality === "normal"
//                         ? "border-blue-500 bg-blue-50 text-blue-600"
//                         : "border-gray-200 text-gray-600 hover:border-blue-300"
//                       }`}
//                   >
...
//                     className={`flex-1 py-3 px-4 rounded-xl border-2 font-medium transition-all duration-200 ${imageQuality === "high"
//                         ? "border-blue-500 bg-blue-50 text-blue-600"
//                         : "border-gray-200 text-gray-600 hover:border-blue-300"
//                       }`}
//                   >
...
//                       <div className="text-xs mt-1">Larger files</div>
//                     </div>
//                   </button>
//                 </div>
//               </div>
...
//                   <div className="space-y-2 text-sm">
//                     <div className="flex items-center justify-between">
//                       <span className="text-blue-700">Files selected:</span>
//                       <span className="font-semibold text-blue-900">{files.length}</span>
//                     </div>
//                     <div className="flex items-center justify-between">
//                       <span className="text-blue-700">Total size:</span>
//                       <span className="font-semibold text-blue-900">{totalSize} MB</span>
//                     </div>
//                     <div className="flex items-center justify-between">
//                       <span className="text-blue-700">Total pages:</span>
//                       <span className="font-semibold text-blue-900">{totalPages} pages</span>
//                     </div>
//                     <div className="flex items-center justify-between">
//                       <span className="text-blue-700">Output format:</span>
//                       <span className="font-semibold text-blue-900">JPG Images</span>
//                     </div>
//                   </div>
//                 </div>
//               )}
...
//           <div className="flex-shrink-0 p-6 border-t bg-gray-50 sticky bottom-0">
//             {!limitsExceeded.hasAnyExceeded ? (
//               <button
//                 onClick={handleConvert}
//                 disabled={files.length === 0}
...
//                   Please reduce files, size, or pages to continue.
//                 </p>
//               </div>
//             )}
...
//                   {/* Page to JPG Option */}
//                   <div
//                     className={`border-2 rounded-xl p-4 cursor-pointer transition-all duration-200 ${conversionMode === "pages"
...
//                   {/* Extract Images Option */}
//                   <div
//                     className={`border-2 rounded-xl p-4 cursor-pointer transition-all duration-200 ${conversionMode === "images"
//                         ? "border-blue-500 bg-blue-50"
//                         : "border-gray-200 hover:border-blue-300"
//                       }`}
//                     onClick={() => setConversionMode("images")}
...
//         {/* Mobile Footer - Updated mobile footer to match desktop behavior */}
//         <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-50">
//           <div className="p-4">
//             <div className="flex items-center gap-3">
...
//               {/* Convert Button or Limits Message */}
//               <div className="flex-1">
//                 {!limitsExceeded.hasAnyExceeded ? (
//                   <button
//                     onClick={handleConvert}
...
// //               {/* Page to JPG Option */}
// //               <div
// //                 className={`border-2 rounded-xl p-4 cursor-pointer transition-all duration-200 ${conversionMode === "pages" ? "border-red-500 bg-red-50" : "border-gray-200 hover:border-red-300"
// //                   }`}
// //                 onClick={() => setConversionMode("pages")}
// //               >
// //                 <div className="flex items-start gap-3">
// //                   <div className="flex-shrink-0 mt-1">
// //                     <div
// //                       className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${conversionMode === "pages" ? "border-red-500 bg-red-500" : "border-gray-300"
// //                         }`}
// //                     >
// //                       {conversionMode === "pages" && (
// //                         <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
...
// //                     </div>
// //                     <p className="text-sm text-gray-600">
// //                       Every page of this PDF will be converted into a JPG file. {estimatedJPGCount} JPG will be created.
// //                     </p>
// //                   </div>
// //                 </div>
// //               </div>
...
// //               {/* Extract Images Option */}
// //               <div
// //                 className={`border-2 rounded-xl p-4 cursor-pointer transition-all duration-200 ${conversionMode === "images" ? "border-red-500 bg-red-50" : "border-gray-200 hover:border-red-300"
// //                   }`}
// //                 onClick={() => setConversionMode("images")}
// //               >
// //                 <div className="flex items-start gap-3">
// //                   <div className="flex-shrink-0 mt-1">
// //                     <div
// //                       className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${conversionMode === "images" ? "border-red-500 bg-red-500" : "border-gray-300"
// //                         }`}
// //                     >
// //                       {conversionMode === "images" && (
// //                         <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
...
// //                     <div className="flex items-center gap-2 mb-2">
// //                       <Download className="w-5 h-5 text-gray-600" />
// //                       <h4 className="font-semibold text-red-600">EXTRACT IMAGES</h4>
// //                     </div>
// //                     <p className="text-sm text-gray-600">
// //                       All embedded images inside the PDF will be extracted as JPG images.
// //                     </p>
...
// //                 <span className="font-semibold text-gray-900">JPG Images</span>
// //               </div>
// //               <div className="flex items-center justify-between text-sm">
// //                 <span className="text-gray-600">Estimated JPGs:</span>
// //                 <span className="font-semibold text-gray-900">{estimatedJPGCount}</span>
// //               </div>
// //               {passwordProtectedFiles.size > 0 && (
// //                 <div className="flex items-center justify-between text-sm">
