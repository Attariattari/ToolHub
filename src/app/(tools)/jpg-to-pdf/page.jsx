...
import { ImageIcon, X, ArrowRight, RotateCw, GripVertical, ChevronDown, Settings } from "lucide-react"
...
const FileInfoSection = ({ files, totalSize, pageOrientation, pageSize, mergeAllImages, pageSizeOptions }) => (
  <div className="mb-6">
    ...
    <div className="space-y-2 text-sm">
      {[
        ...
        ["Page size:", pageSizeOptions.find((opt) => opt.value === pageSize)?.label.split(" ")[0]],
        ["Output:", mergeAllImages ? "1 PDF file" : `${files.length} PDF files`]
      ].map(([label, value]) => (
...
const Sidebar = ({
  ...
  <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
    <div className="p-6 flex-1">
      <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Image to PDF Converter</h3>

      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <p className="text-sm text-blue-800">
          Convert JPG images to PDF in seconds. Easily adjust orientation and margins with drag-and-drop reordering.
        </p>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`flex-1 p-4 rounded-xl border-2 transition-all duration-200 ${pageOrientation === "portrait" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"
              }`}
          >
            <div className="flex flex-col items-center gap-2">
              <div className={`w-8 h-10 border-2 rounded ${pageOrientation === "portrait" ? "border-blue-500" : "border-gray-400"}`}></div>
              <span className={`text-sm font-medium ${pageOrientation === "portrait" ? "text-blue-600" : "text-gray-600"}`}>Portrait</span>
            </div>
          </button>
          <button
            onClick={() => setPageOrientation("landscape")}
            className={`flex-1 p-4 rounded-xl border-2 transition-all duration-200 ${pageOrientation === "landscape" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"}`}
          >
            <div className="flex flex-col items-center gap-2">
              <div className={`w-10 h-8 border-2 rounded ${pageOrientation === "landscape" ? "border-blue-500" : "border-gray-400"}`}></div>
              <span className={`text-sm font-medium ${pageOrientation === "landscape" ? "text-blue-600" : "text-gray-600"}`}>Landscape</span>
            </div>
          </button>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 p-3 rounded-xl border-2 transition-all duration-200 ${margin === "none" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"}`}
          >
            <div className="flex flex-col items-center gap-2">
              <ImageIcon className={`w-6 h-6 ${margin === "none" ? "text-blue-500" : "text-gray-400"}`} />
              <span className={`text-sm font-medium ${margin === "none" ? "text-blue-600" : "text-gray-600"}`}>No margin</span>
            </div>
          </button>
          <button
            onClick={() => setMargin("small")}
            className={`flex-1 p-3 rounded-xl border-2 transition-all duration-200 ${margin === "small" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"}`}
          >
            <div className="flex flex-col items-center gap-2">
              <div className={`w-6 h-6 border-2 border-dashed rounded ${margin === "small" ? "border-blue-500" : "border-gray-400"}`}></div>
              <span className={`text-sm font-medium ${margin === "small" ? "text-blue-600" : "text-gray-600"}`}>Small</span>
            </div>
          </button>
          <button
            onClick={() => setMargin("big")}
            className={`flex-1 p-3 rounded-xl border-2 transition-all duration-200 ${margin === "big" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"}`}
          >
            <div className="flex flex-col items-center gap-2">
              <div className={`w-6 h-6 border-4 border-dashed rounded ${margin === "big" ? "border-blue-500" : "border-gray-400"}`}></div>
              <span className={`text-sm font-medium ${margin === "big" ? "text-blue-600" : "text-gray-600"}`}>Big</span>
            </div>
          </button>
        </div>
      </div>
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <div className="relative">
            <input
              ...
              className="sr-only"
            />
            <div className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-all duration-200 ${mergeAllImages ? "bg-green-500 border-green-500" : "border-gray-300"}`}>
              {mergeAllImages && (
      ...
      {files.length > 0 && <FileInfoSection files={files} totalSize={totalSize} pageOrientation={pageOrientation} pageSize={pageSize} mergeAllImages={mergeAllImages} pageSizeOptions={pageSizeOptions} />}
    </div>
    ...
    <div className="flex-shrink-0 p-4 border-t bg-gray-50 sticky bottom-4">
      ...
      {files.length === 0 && <p className="text-xs text-gray-500 text-center mt-2">Select image files to convert</p>}
    </div>
  </div>
)
...
export default function JPGToPDFPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const downloadUrl = `/downloads/document=${encodedZipPath}?dbTaskId=${response.data.data.dbTaskId}&tool-type=jpg-to-pdf`
  ...
  const sidebarProps = {
    files, totalSize, limitsExceeded, onConvert: handleConvert,
    pageOrientation, setPageOrientation, pageSize, setPageSize,
    margin, setMargin, mergeAllImages, setMergeAllImages, pageSizeOptions
  }
