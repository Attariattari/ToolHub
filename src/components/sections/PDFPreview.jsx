...
const PDFPreview = memo(
  ({
    ...
    highlightStyle = "modern", // modern, classic, minimal
  }) => {
      ...
      return (
        ...
        >
          {currentPageDifferences.map((diff, index) => {
            ...
            const highlightTitle = `${diff.changeType === "removed" ? "Removed" : "Added"
              }: "${diff.text.substring(0, 60)}${diff.text.length > 60 ? "..." : ""
              }" (${confidencePercent}% confidence, ${diff.matchType || "exact"
              } match)`;
            ...
            return (
              <div
                key={diff.uniqueId}
                className={`absolute transition-all duration-300 ${animateHighlights ? "highlight-animation" : ""
                  }`}
                style={{
          ...
          <div className="w-full h-full bg-white relative">
            {!isLoading ? (
              ...
              >
                {documentLoaded && (
                  <div
                    className={`relative ${isSinglePage
                        ? userZoom > 100
