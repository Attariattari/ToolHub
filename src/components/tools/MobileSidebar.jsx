...
export default function MobileSidebar({
  ...
  const buttonColorClasses = {
    red: "bg-blue-600 hover:bg-blue-700",
        ...
        <div className="p-4">
          <div className="p-6">
            ...
            {warnings.map((warning, index) => (
              <div key={index} className={`rounded-xl p-4 mb-6 ${warning.type === 'yellow' ? 'bg-yellow-50' : 'bg-blue-50'}`}>
                <p className={`text-sm ${warning.type === 'yellow' ? 'text-yellow-800' : 'text-blue-800'}`}>
                  {warning.message}
                </p>
              </div>
            ))}
