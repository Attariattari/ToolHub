...
export default function PasswordModal({ isOpen, onClose, passwordProtectedFiles, onSubmit }) {
  ...
  const validatePassword = useCallback(async (file, password, fileId) => {
    ...
    try {
      ...
      if (fileName.endsWith(".pdf")) {
        // ✅ PDF: Set checking state before async validation
        ...
        try {
          ...
          setValidationErrors((prev) => ({
            ...prev,
            [fileId]: "Invalid PDF password.",
          }))
        }
      } else {
        // ✅ Office/other: Directly mark as valid, no loading state
        setPasswordStatus((prev) => ({ ...prev, [fileId]: "valid" }))
        setValidationErrors((prev) => ({ ...prev, [fileId]: null }))
      }
    } catch (err) {
      ...
      setValidationErrors((prev) => ({
        ...prev,
        [fileId]: "Unexpected error occurred during password check.",
      }))
    }
  }, [])
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const allFilesHavePasswords = passwordProtectedFiles.every(
      (file) => passwords[file.id] && passwords[file.id].trim() !== "",
    )
        ...
        <div className="p-6 space-y-4">
          <div className="bg-blue-50 rounded-lg p-3 mb-4">
            <p className="text-sm text-blue-800">
              {passwordProtectedFiles.length} file{passwordProtectedFiles.length > 1 ? "s" : ""} require
              {passwordProtectedFiles.length === 1 ? "s" : ""} password{passwordProtectedFiles.length > 1 ? "s" : ""} to
              process.
            </p>
          </div>
              ...
              <div key={file.id} className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    disabled={isSubmitting}
                    className={`w-full p-3 pr-20 border rounded-lg bg-gray-50 focus:bg-white focus:ring-2 outline-none transition-all duration-200 ${inputStatus?.borderColor || "border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                      }`}
                  />

                  {/* Status Icon */}
                  <div className="absolute right-12 top-1/2 transform -translate-y-1/2">{inputStatus?.icon}</div>
                ...
                {/* Success Message */}
                {passwordStatus[file.id] === "valid" && file.name.toLowerCase().endsWith(".pdf") && (
                ...
                {passwordStatus[file.id] === "valid" && !file.name.toLowerCase().endsWith(".pdf") && (
                  <p className="text-xs text-gray-700 italic ml-1">⚠️ Make sure your password is correct.</p>
                )}

                {/* Checking Message */}
                {passwordStatus[file.id] === "checking" && file.name.toLowerCase().endsWith(".pdf") && (
                  <p className="text-sm text-blue-600 flex items-center gap-1">
        ...
        <div className="p-6 border-t">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                Process Files
                {!isFormValid && <span className="text-xs opacity-75 ml-1">(Enter valid passwords)</span>}
              </>
            )}
          </button>
        </div>
...
//         } catch (err) {
//           setPasswordStatus(prev => ({ ...prev, [fileId]: 'invalid' }))
//           setValidationErrors(prev => ({ ...prev, [fileId]: 'Invalid PDF password.' }))
//         }
...
//                 {passwordStatus[file.id] === 'valid' && !file.name.toLowerCase().endsWith(".pdf") && (
//                   <p className="text-xs text-gray-700 italic ml-1">
//                     ⚠️ Make sure your password is correct.
//                   </p>
//                 )}
